...
function setTranslationResult(
  ...
  const copyBtn = section.querySelector(
    ".qt-ocr-result__tl-copy",
...
function showOcrResultPanel(
  ...
  const previewSection = croppedUrl
    ...
    <div class="qt-ocr-result__meta qt-ocr-result__meta--hidden">Confidence: ${confidence}%${elapsedSuffix}</div>
  `
    : "";

  // No separate metaSection — it's now part of previewSection and toggles with the image
  const metaSection = "";
  ...
  panel
    .querySelector(".qt-ocr-result__close")
    ?.addEventListener("click", removeOcrResultPanel);

  // Image toggle — also toggles meta section (confidence and processing time)
  const toggleBtn = panel.querySelector(
    ".qt-ocr-result__preview-toggle",
  ) as HTMLButtonElement | null;
  const previewImg = panel.querySelector(
    ".qt-ocr-result__preview",
  ) as HTMLImageElement | null;
  const metaDiv = panel.querySelector(
    ".qt-ocr-result__meta",
  ) as HTMLDivElement | null;
  if (toggleBtn && previewImg) {
    toggleBtn.addEventListener("click", () => {
      const hidden = previewImg.classList.toggle(
        "qt-ocr-result__preview--hidden",
      );
      if (metaDiv) {
        metaDiv.classList.toggle("qt-ocr-result__meta--hidden", hidden);
      }
  ...
  const translateBtn = panel.querySelector(
    ".qt-ocr-result__translate-btn",
  ) as HTMLButtonElement | null;
...
function showOcrLoadingPanel(): void {
  ...
  panel
    .querySelector(".qt-ocr-result__close")
    ?.addEventListener("click", cancelOcr);
  panel
    .querySelector(".qt-ocr-result__cancel")
    ?.addEventListener("click", cancelOcr);
...
function startOcrSelection(): void {
  removeOcrOverlay();
  removeOcrResultPanel();
  ...
  function onMouseUp(e: MouseEvent): void {
    ...
    showOcrLoadingPanel();
    chrome.runtime.sendMessage({ type: MSG_SELECTION_DONE, rect, dpr });
  }
